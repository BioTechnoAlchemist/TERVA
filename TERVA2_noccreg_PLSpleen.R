...
sc <- load10X("/Users/limikk/Desktop/TERVA/TERVA2DATA/PL_Spleen_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/PL_Spleen_GEX_Ab_VDJ_noIntr_SC5PPE/count", includeFeatures = "Gene Expression") #This will only include the gene expression data. Include ab data later.
sc

# Channel with 32285 genes and 5882 cells (PL Spleen)
...
#702 genes passed tf-idf cut-off and 265 soup quantile filter.  Taking the top 100.
#Using 508 independent estimates of rho.
#Estimated global rho of 0.01
#Channel with 32285 genes and 5882 cells
...
sc <- calculateContaminationFraction(sc, list(IG = genes), useToEst = useToEst) #Estimated global contamination fragtion of 5.14%
#head(sc$metaData)
...
#### ADD antibody data ####
tenx <- Read10X("/Users/limikk/Desktop/TERVA/TERVA2DATA/PL_Spleen_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/PL_Spleen_GEX_Ab_VDJ_noIntr_SC5PPE/count/filtered_feature_bc_matrix/")
...
plot1 <- DimPlot(srat, group.by="DF.classifications_0.25_0.005_248", reduction="umap", pt.size=1, order=c("Coll.Duct.TC","Doublet"), cols=c("#66C2A5","#FFD92F","#8DA0CB"))
plot2 <- DimPlot(srat, reduction = "umap", pt.size=1, group.by = "RNA_snn_res.0.5")

plot1 + plot2 
#plot1 + plot1_1

srat_DF <- subset(srat, subset = DF.classifications_0.25_0.005_248 == "Singlet") #Creates a new Seurat Object from the singlets.

plot3 <- DimPlot(srat_DF, group.by="DF.classifications_0.25_0.005_248", reduction="umap", pt.size=0.5, order=c("Coll.Duct.TC","Doublet"), cols=c("#66C2A5","#FFD92F","#8DA0CB"))
...
srat_N <- FindClusters(srat_N, resolution = 0.6, verbose = F)
plot7 <- DimPlot(srat_N, reduction = "umap", pt.size=0.5, label = T, group.by = "RNA_snn_res.0.6")
FeaturePlot(srat_N, features = c("Ccr5", "Lyz2", "H2-Ab1", "Tlr2", "Ltb", "Cd79a", "Cd3d", "Ccr2", "Cx3cr1", "Treml4", "Ly6g", "Siglecf", "Clec9a", "Sirpa", "Xcr1", "Siglech", "Cd27", "Itgam"), pt.size = 0.2,
            ncol = 4) #Normal spleen should have appr. 21-25 % T cells, 44-58 % B cells, 3.5-5 % monocytes, 1-2 % granulocytes, 1-3 %, 1-3 % DC's, 1-2 % NK's and 1-2 % macrophages.
...
VlnPlot(srat_N, features = c("Ccr5", "Lyz2", "H2-Ab1", "Tlr2", "Ltb", "Cd79a", "Cd3d", "Ccr2", "Cx3cr1", "Treml4", "Ly6g", "Siglecf", "Clec9a", "Sirpa", "Xcr1", "Siglech", "Cd27", "Itgam"), pt.size = 0.2, ncol = 3)
plot9 <- DotPlot(srat_N, features = c("Ccr5", "Lyz2", "H2-Ab1", "Tlr2", "Ltb", "Cd79a", "Cd3d", "Ccr2", "Cx3cr1", "Treml4", "Ly6g", "Siglecf", "Clec9a", "Sirpa", "Xcr1", "Siglech", "Cd27", "Itgam")) + RotatedAxis()
plot10 <- DoHeatmap(subset(srat_N, downsample = 100), features = c("Ccr5", "Lyz2", "H2-Ab1", "Tlr2", "Ltb", "Cd79a", "Cd3d", "Ccr2", "Cx3cr1", "Treml4", "Ly6g", "Siglecf", "Clec9a", "Sirpa", "Xcr1", "Siglech", "Cd27", "Itgam"), size = 3)
...
srat_TCR <- import_vdj(input = srat_N, vdj_dir = "~/Desktop/TERVA/TERVA2DATA/PL_Spleen_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/PL_Spleen_GEX_Ab_VDJ_noIntr_SC5PPE/vdj_t/", filter_paired = F)
srat_BCR <- import_vdj(input = srat_N, vdj_dir = "~/Desktop/TERVA/TERVA2DATA/PL_Spleen_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/PL_Spleen_GEX_Ab_VDJ_noIntr_SC5PPE/vdj_b/", filter_paired = F)

srat_VDJ <- import_vdj(input = srat_N, vdj_dir = "~/Desktop/TERVA/TERVA2DATA/PL_Spleen_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/PL_Spleen_GEX_Ab_VDJ_noIntr_SC5PPE/vdj_t/", prefix = "TCR_", filter_paired = F)
srat_VDJ <- import_vdj(input = srat_VDJ, vdj_dir = "~/Desktop/TERVA/TERVA2DATA/PL_Spleen_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/PL_Spleen_GEX_Ab_VDJ_noIntr_SC5PPE/vdj_b/", prefix = "BCR_", filter_paired = F)
...
library(rjson)
Tcells <- as.data.frame(fromJSON(file = "~/Desktop/TERVA/TERVA2DATA/PL_Spleen_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/PL_Spleen_GEX_Ab_VDJ_noIntr_SC5PPE/vdj_t/cell_barcodes.json"))
names(Tcells)[1] <- "Tcells" 
Bcells <- as.data.frame(fromJSON(file = "~/Desktop/TERVA/TERVA2DATA/PL_Spleen_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/PL_Spleen_GEX_Ab_VDJ_noIntr_SC5PPE/vdj_b/cell_barcodes.json"))
names(Bcells)[1] <- "Bcells" 

Tcells<- as.vector(Tcells)
Bcells<- as.vector(Bcells)
Tc <- DimPlot(object = srat_VDJ, cells.highlight = Tcells, cols.highlight = "red", cols = "gray", order = TRUE, group.by = "RNA_snn_res.0.6", label = T)
Bc <- DimPlot(object = srat_VDJ, cells.highlight = Bcells, cols.highlight = "blue", cols = "gray", order = TRUE, group.by = "RNA_snn_res.0.6", label = T)
Tc + Bc
...
arrange(srat12markers,
        desc(avg_log2FC)) %>%
  rownames()

sratmarkers <- FindMarkers(srat_N, ident.1 = 2, min.pct = 0.1) 
top2 <- head(sratmarkers, 10)
FeaturePlot(srat_N, features = rownames(top2))


FeaturePlot(srat_N, features = "Ptprc")
...
srat0markers <- FindMarkers(srat_N, ident.1 = 0, min.pct = 0.1) # "Cd8b1"          "Cd8a"           "Dapl1"          "Dnajc15"        "Il7r"           "Lef1"           "Arl4c"          "Thy1"           "Fam241a"        "Saraf" 
srat1markers <- FindMarkers(srat_N, ident.1 = 1, min.pct = 0.1) # "Igfbp4"        "Ly6c1"         "Lef1"          "Cd4"           "Actn1"         "Tcf7"          "Trbv31"        "Satb1"         "Dgka"          "Npc2"          "Ccr7" 
srat2markers <- FindMarkers(srat_N, ident.1 = 2, min.pct = 0.1) # "Cd74"           "Cd79a"          "H2-Aa"          "Ighd"           "Cd79b"          "H2-Eb1"         "Ebf1"           "Iglc3"          "H2-Ab1"         "Fcmr"           "Bank1" 
srat3markers <- FindMarkers(srat_N, ident.1 = 3, min.pct = 0.1) # "Ly6d"           "Cd79a"          "Ms4a1"          "Cd79b"          "Iglc2"          "Cd74"           "Iglc3"          "Fcmr"           "Bank1"          "Cr2"            "Ebf1"   
srat4markers <- FindMarkers(srat_N, ident.1 = 4, min.pct = 0.1) # "Izumo1r"       "Ikzf2"         "Itgb1"         "Maf"           "Tnfrsf4"       "Cd4"           "Foxp3"         "Tbc1d4"        "Cd5"           "Hif1a"         "Ifi27l2a"
srat5markers <- FindMarkers(srat_N, ident.1 = 5, min.pct = 0.1) # "Ccl5"           "Ly6c2"          "Nkg7"           "Ctla2a"         "Ctsw"           "Il2rb"          "Hopx"           "Gzmk"           "Eomes"          "Ifitm10"        "Klrd1" 
srat6markers <- FindMarkers(srat_N, ident.1 = 6, min.pct = 0.1) # "Hist1h2ap"     "Mki67"         "Hmgb2"         "Hist1h2ae"     "Pclaf"         "Hist1h1b"      "Stmn1"         "Top2a"         "H2afx"         "Hmgn2"         "Tuba1b"
srat7markers <- FindMarkers(srat_N, ident.1 = 7, min.pct = 0.1) # "Gzma"          "Ncr1"          "Ccl5"          "Klrb1c"        "Fcer1g"        "Klra8"         "Klrk1"         "Nkg7"          "Tyrobp"        "Cd7"           "AW112010" 
srat8markers <- FindMarkers(srat_N, ident.1 = 8, min.pct = 0.1) # "Cst3"          "Ccnd1"         "Tmem176b"      "Plbd1"         "Apol7c"        "Ifi30"         "H2-Ab1"        "H2-Eb1"        "S100a4"        "Tmem176a"      "H2-Aa" 
srat9markers <- FindMarkers(srat_N, ident.1 = 9, min.pct = 0.1) # "Lyz2"          "Ifitm3"        "Apoe"          "Fcer1g"        "Cebpb"         "Cst3"          "Gngt2"         "Lgals3"        "Lst1"          "Csf1r"         "Cybb"
srat10markers <- FindMarkers(srat_N, ident.1 = 10, min.pct = 0.1) # "S100a9"        "S100a8"        "Retnlg"        "Ccl6"          "Il1b"          "Csf3r"         "Slpi"          "Ifitm1"        "Msrb1"         "Tyrobp"        "Ifitm2"
srat11markers <- FindMarkers(srat_N, ident.1 = 11, min.pct = 0.1) # "Igkv4-55"       "Jchain"         "Igkv5-39"       "Ighm"           "Igkc"           "Iglv1"          "Igha"           "Iglv3"          "Mzb1"           "Txndc5"         "Ighg3"
srat12markers <- FindMarkers(srat_N, ident.1 = 12, min.pct = 0.1) # "Cxcr6"         "Id2"           "Ckb"           "Il2rb"         "Trbv13-2"      "Xcl1"          "Cd160"         "Trgv2"         "AW112010"      "Cd226"         "Cxcr3"




srat10vs2markers <- FindMarkers(srat_N, ident.1 = 10, ident.2 = 2, min.pct = 0.1) #
srat9vs2markers <- FindMarkers(srat_N, ident.1 = 9, ident.2 = 2, min.pct = 0.1) #
...
h5<-LoadH5Seurat("PLSpleenafterQC.h5Seurat") #works, srat_VDJ and h5 look similar (small changes in the order of items though and md5 sums are different but let's see if they output similar results)

remove(list = c("srat_BCR","srat_TCR","h5","sub","out","h5md5","vdjmd5","plot7", "plot7b", "vdjmarkers", "h5markers"))

#h5md5<- digest(h5)
#vdjmd5<- digest(srat_VDJ)

#plot7 <- DimPlot(srat_VDJ, reduction = "umap", pt.size=0.5, label = T, group.by = "RNA_snn_res.0.6")
#plot7b <-DimPlot(h5, reduction = "umap", pt.size=0.5, label = T, group.by = "RNA_snn_res.0.6")

#plot7 + plot7b #looks the same
#vdjmarkers <- FindMarkers(srat_VDJ, ident.1 = 2, min.pct = 0.1) 
#h5markers <- FindMarkers(h5, ident.1 = 2, min.pct = 0.1) # These look the same also, so the md5 sum probably differs due to the small changes in the order of the data within the dataobject.

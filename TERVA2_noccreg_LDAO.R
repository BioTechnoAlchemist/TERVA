...
sc <- load10X("/Users/limikk/Desktop/TERVA/TERVA2DATA/LD_AO_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/LD_AO_GEX_Ab_VDJ_noIntr_SC5PPE/count", includeFeatures = "Gene Expression") #This will only include the gene expression data. Include ab data later.
sc

# Channel with 32285 genes and 8847 cells (LD AO; this is really the sample LD_eWAT)
...
#1401 genes passed tf-idf cut-off and 419 soup quantile filter.  Taking the top 100.
#Using 540 independent estimates of rho.
#Estimated global rho of 0.03
#Channel with 32423 genes and 8847 cells
# 3 % SHOULD BE ENOUGH, WE DON'T WANT TO BE TOO CONCERVATIVE
...
#### ADD antibody data ####
tenx <- Read10X("/Users/limikk/Desktop/TERVA/TERVA2DATA/LD_AO_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/LD_AO_GEX_Ab_VDJ_noIntr_SC5PPE/count/filtered_feature_bc_matrix/")
...
plot1 <- DimPlot(srat, group.by="DF.classifications_0.25_0.005_381", reduction="umap", pt.size=1, order=c("Coll.Duct.TC","Doublet"), cols=c("#66C2A5","#FFD92F","#8DA0CB"))
plot2 <- DimPlot(srat, reduction = "umap", pt.size=1, group.by = "RNA_snn_res.0.5")

plot1 + plot2 
#plot1 + plot1_1

srat_DF <- subset(srat, subset = DF.classifications_0.25_0.005_381 == "Singlet") #Creates a new Seurat Object from the singlets.

plot3 <- DimPlot(srat_DF, group.by="DF.classifications_0.25_0.005_381", reduction="umap", pt.size=0.5, order=c("Coll.Duct.TC","Doublet"), cols=c("#66C2A5","#FFD92F","#8DA0CB"))
plot4 <- DimPlot(srat_DF, reduction = "umap", pt.size=0.5, label = T, group.by = "RNA_snn_res.0.7")
...
FeaturePlot(srat_N, features = c("Myh11", "Cnn1", "Tcf21", "C1qa", "Lyz2", "H2-Ab1", "Vwf", "Flt1", "Kdr", "Ltb", "Pdgfra", "Lum", "Pecam1", "Cdh5", "Cd79a", "Cd3d", "Cnp"), pt.size = 0.2,
            ncol = 4)
FeaturePlot(srat_N, features = c("Il7r", "Icos", "Kit","Tcf21"), pt.size = 0.2,
...
srat_TCR <- import_vdj(input = srat_N, vdj_dir = "~/Desktop/TERVA/TERVA2DATA/LD_AO_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/LD_AO_GEX_Ab_VDJ_noIntr_SC5PPE/vdj_t/", filter_paired = F)
srat_BCR <- import_vdj(input = srat_N, vdj_dir = "~/Desktop/TERVA/TERVA2DATA/LD_AO_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/LD_AO_GEX_Ab_VDJ_noIntr_SC5PPE/vdj_b/", filter_paired = F)

srat_VDJ <- import_vdj(input = srat_N, vdj_dir = "~/Desktop/TERVA/TERVA2DATA/LD_AO_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/LD_AO_GEX_Ab_VDJ_noIntr_SC5PPE/vdj_t/", prefix = "TCR_", filter_paired = F)
srat_VDJ <- import_vdj(input = srat_VDJ, vdj_dir = "~/Desktop/TERVA/TERVA2DATA/LD_AO_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/LD_AO_GEX_Ab_VDJ_noIntr_SC5PPE/vdj_b/", prefix = "BCR_", filter_paired = F)
...
library(rjson)
Tcells <- as.data.frame(fromJSON(file = "~/Desktop/TERVA/TERVA2DATA/LD_AO_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/LD_AO_GEX_Ab_VDJ_noIntr_SC5PPE/vdj_t/cell_barcodes.json"))
names(Tcells)[1] <- "Tcells" 
Bcells <- as.data.frame(fromJSON(file = "~/Desktop/TERVA/TERVA2DATA/LD_AO_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/LD_AO_GEX_Ab_VDJ_noIntr_SC5PPE/vdj_b/cell_barcodes.json"))
...
srat0markers <- FindMarkers(srat_N, ident.1 = 0, min.pct = 0.1) # Ccl8, Pf4, F13a1, Mrc1, Retnla, Cbr2, Folr2, C1qa, C1qc, Cd209f
srat1markers <- FindMarkers(srat_N, ident.1 = 1, min.pct = 0.1) # Dcn, Sult1e1, Gsn, Gpx3, Penk, Inmt, Apod, Serping1, Clec3b, C3, Ccl11  
srat2markers <- FindMarkers(srat_N, ident.1 = 2, min.pct = 0.1) # Lgals3, Mmp12,Cxcl2, Lyz1, Spp1, Trem2, Gpnmb, Ctss, Lyz2
srat3markers <- FindMarkers(srat_N, ident.1 = 3, min.pct = 0.1) # Gpihbp1, Fabp4, Cldn5, Mgll, Cav1, Rbp7, Cdh5, Cavin2, Rflnb, Car4
srat4markers <- FindMarkers(srat_N, ident.1 = 4, min.pct = 0.1) # Fbn1, Mfap5, Col3a1, Col1a1, Pi16, Igfbp6, Fstl1, Col1a2
srat5markers <- FindMarkers(srat_N, ident.1 = 5, min.pct = 0.1) # "Fabp4"         "Gpihbp1"       "Rbp7"          "Cldn5"         "Ptprb"         "Ly6c1"         "Gng11"         "Aqp1"          "Cavin2"
srat6markers <- FindMarkers(srat_N, ident.1 = 6, min.pct = 0.1) # Plbd1, Ccr7,Lsp1, H2-Ab1, H2-Aa, Cd74, Cst3, H2-Eb1, Gm2a
srat7markers <- FindMarkers(srat_N, ident.1 = 7, min.pct = 0.1) # "Jchain"        "Igkc"          "Cd79a"         "Ighm"          "Iglv1"         "Ly6d"          "Iglc2"         "Cd79b"         "Mzb1"          "Ms4a1"    
srat8markers <- FindMarkers(srat_N, ident.1 = 8, min.pct = 0.1) # "Ccl5"          "Ms4a4b"        "Cd3e"          "Cd3d"          "Cd3g"          "Vps37b"        "Lat"           "Nkg7"          "Gramd3"        "Trbc2"  
srat9markers <- FindMarkers(srat_N, ident.1 = 9, min.pct = 0.1) # "Plac8"          "Gngt2"          "Apoc2"          "Lst1"           "Cebpb"          "Cybb"           "Ear2"           "Pglyrp1"        "Il1b" 
srat10markers <- FindMarkers(srat_N, ident.1 = 10, min.pct = 0.1) # "Rgs5"          "Kcnj8"         "Ndufa4l2"      "Fabp4"         "Timp3"         "Cox4i2"        "Abcc9"         "Postn"         "Gng11"         "Gpihbp1" 
srat11markers <- FindMarkers(srat_N, ident.1 = 11, min.pct = 0.1) # "Csf2"          "Areg"          "Hilpda"        "Il7r"          "Trdv4"         "Ltb"           "Cxcr6"         "Tnfaip3"       "Il2rg"         "Rgs2"  
srat12markers <- FindMarkers(srat_N, ident.1 = 12, min.pct = 0.1) # "Gzma"          "Ccl5"          "Nkg7"          "Xcl1"          "Ly6c2"         "AW112010"      "Gzmb"          "Ncr1"          "Ctsw"          "Il2rb"      
srat13markers <- FindMarkers(srat_N, ident.1 = 13, min.pct = 0.1) # "Hist1h2ae"     "Hist1h2ap"     "Hmgb2"         "Ube2c"         "Stmn1"         "Mki67"         "Top2a"         "Birc5"         "Tubb5"         "Pclaf" 
srat14markers <- FindMarkers(srat_N, ident.1 = 14, min.pct = 0.1) # "Malat1"        "Gfpt2"         "Tnfaip6"       "Fosb"          "Meg3"          "Errfi1"        "Kcnq1ot1"      "Cd55"          "Neat1"         "Tnxb"
srat15markers <- FindMarkers(srat_N, ident.1 = 15, min.pct = 0.1) # "S100a9"        "S100a8"        "Retnlg"        "Slpi"          "Ifitm1"        "Il1b"          "Msrb1"         "Csf3r"         "Acod1"         "Hdc" 
srat16markers <- FindMarkers(srat_N, ident.1 = 16, min.pct = 0.1) # "Acta2"         "Tagln"         "Myl9"          "Myh11"         "Tpm2"          "Tpm1"          "Mustn1"        "Mylk"          "Gm13889"       "Igfbp5" 
srat17markers <- FindMarkers(srat_N, ident.1 = 17, min.pct = 0.1) # "Acta2"         "Tagln"         "Myl9"          "Csrp2"         "Clu"           "Cnn1"          "Tpm2"          "Ccn2"          "Myh11"         "Cpe"


srat16vs17markers <- FindMarkers(srat_N, ident.1 = 16, ident.2 = 17, min.pct = 0.1) #Mgp, Clu, Nupr1, Ptgis, Sost, Bgn, Rbp1, Ccn3, Csrp2, Pam
srat7vs8markers <- FindMarkers(srat_N, ident.1 = 8, ident.2 = 7, min.pct = 0.1) #Mgp, Clu, Nupr1, Ptgis, Sost, Bgn, Rbp1, Ccn3, Csrp2, Pam
...
LDeWAT_h5<-LoadH5Seurat("LD_eWATafterQC.h5Seurat") #works, srat_VDJ and LDeWAT_h5 look similar

#remove(list = c("srat_BCR","srat_TCR","LDeWAT_h5","sub","out"))

...
sc <- load10X("~/path", includeFeatures = "Gene Expression")
sc
...
#### ADD antibody data ####
tenx <- Read10X("/Users/limikk/Desktop/TERVA/TERVA2DATA/LD_eWAT_GEX_Ab_VDJ_noIntr_SC5PPE_actuallyLDAO/outs/per_sample_outs/LD_eWAT_GEX_Ab_VDJ_noIntr_SC5PPE/count/filtered_feature_bc_matrix")
...
#Subset the suspicious (double positive) cluster 1

srat_NO1 <- subset(srat_N, subset = seurat_clusters %in% c(0,2:9,12,14,16))

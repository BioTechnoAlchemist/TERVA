...
sc <- load10X("/Users/limikk/Desktop/TERVA/TERVA2DATA/LD_Spleen_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/LD_Spleen_GEX_Ab_VDJ_noIntr_SC5PPE/count", includeFeatures = "Gene Expression") #This will only include the gene expression data. Include ab data later.
sc

# Channel with 32285 genes and 4658 cells (LD Spleen)
...
#1560 genes passed tf-idf cut-off and 526 soup quantile filter.  Taking the top 100.
#Using 415 independent estimates of rho.
...
sc <- calculateContaminationFraction(sc, list(IG = genes), useToEst = useToEst) #Estimated global contamination fragtion of 6.00%
#head(sc$metaData)
...
#### ADD antibody data ####
tenx <- Read10X("/Users/limikk/Desktop/TERVA/TERVA2DATA/LD_Spleen_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/LD_Spleen_GEX_Ab_VDJ_noIntr_SC5PPE/count/filtered_feature_bc_matrix/")
...
plot1 <- DimPlot(srat, group.by="DF.classifications_0.25_0.005_199", reduction="umap", pt.size=1, order=c("Coll.Duct.TC","Doublet"), cols=c("#66C2A5","#FFD92F","#8DA0CB"))
plot2 <- DimPlot(srat, reduction = "umap", pt.size=1, group.by = "RNA_snn_res.0.5")

plot1 + plot2 
#plot1 + plot1_1

srat_DF <- subset(srat, subset = DF.classifications_0.25_0.005_199 == "Singlet") #Creates a new Seurat Object from the singlets.

plot3 <- DimPlot(srat_DF, group.by="DF.classifications_0.25_0.005_199", reduction="umap", pt.size=0.5, order=c("Coll.Duct.TC","Doublet"), cols=c("#66C2A5","#FFD92F","#8DA0CB"))
...
srat_TCR <- import_vdj(input = srat_N, vdj_dir = "~/Desktop/TERVA/TERVA2DATA/LD_Spleen_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/LD_Spleen_GEX_Ab_VDJ_noIntr_SC5PPE/vdj_t/", filter_paired = F)
srat_BCR <- import_vdj(input = srat_N, vdj_dir = "~/Desktop/TERVA/TERVA2DATA/LD_Spleen_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/LD_Spleen_GEX_Ab_VDJ_noIntr_SC5PPE/vdj_b/", filter_paired = F)

srat_VDJ <- import_vdj(input = srat_N, vdj_dir = "~/Desktop/TERVA/TERVA2DATA/LD_Spleen_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/LD_Spleen_GEX_Ab_VDJ_noIntr_SC5PPE/vdj_t/", prefix = "TCR_", filter_paired = F)
srat_VDJ <- import_vdj(input = srat_VDJ, vdj_dir = "~/Desktop/TERVA/TERVA2DATA/LD_Spleen_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/LD_Spleen_GEX_Ab_VDJ_noIntr_SC5PPE/vdj_b/", prefix = "BCR_", filter_paired = F)
...
library(rjson)
Tcells <- as.data.frame(fromJSON(file = "~/Desktop/TERVA/TERVA2DATA/LD_Spleen_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/LD_Spleen_GEX_Ab_VDJ_noIntr_SC5PPE/vdj_t/cell_barcodes.json"))
names(Tcells)[1] <- "Tcells" 
Bcells <- as.data.frame(fromJSON(file = "~/Desktop/TERVA/TERVA2DATA/LD_Spleen_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/LD_Spleen_GEX_Ab_VDJ_noIntr_SC5PPE/vdj_b/cell_barcodes.json"))
names(Bcells)[1] <- "Bcells" 

Tcells<- as.vector(Tcells)
Bcells<- as.vector(Bcells)
Tc <- DimPlot(object = srat_VDJ, cells.highlight = Tcells, cols.highlight = "red", cols = "gray", order = TRUE, group.by = "RNA_snn_res.0.5", label = T)
Bc <- DimPlot(object = srat_VDJ, cells.highlight = Bcells, cols.highlight = "blue", cols = "gray", order = TRUE, group.by = "RNA_snn_res.0.5", label = T)
Tc + Bc
...
srat0markers <- FindMarkers(srat_N, ident.1 = 0, min.pct = 0.1) # "Igfbp4"        "Ly6c1"         "Lef1"          "Tcf7"          "Cd4"           "Ccr7"          "Trbv31"        "Npc2"          "Lat"           "Dapl1"  
srat1markers <- FindMarkers(srat_N, ident.1 = 1, min.pct = 0.1) # "Cd8b1"         "Cd8a"          "Dapl1"         "Dnajc15"       "Nkg7"          "Ctsw"          "Tgtp1"         "Arl4c"         "Lef1"          "Saraf"  
srat2markers <- FindMarkers(srat_N, ident.1 = 2, min.pct = 0.1) # "Cd79a"          "Ly6d"           "Cd79b"          "Cd74"           "H2-Aa"          "Ebf1"           "Fcmr"           "Iglc3"          "Bank1" 
srat3markers <- FindMarkers(srat_N, ident.1 = 3, min.pct = 0.1) # "Izumo1r"       "Ikzf2"         "Tnfrsf4"       "Itgb1"         "Hif1a"         "Tigit"         "Pdcd1"         "Maf"           "Cd4"           "Ctla4" 
srat4markers <- FindMarkers(srat_N, ident.1 = 4, min.pct = 0.1) # "Ccl5"           "Ly6c2"          "Nkg7"           "Ctla2a"         "Il2rb"          "Ctsw"           "Hopx"           "Cxcr6"          "Ahnak"          "Gzmk"   
srat5markers <- FindMarkers(srat_N, ident.1 = 5, min.pct = 0.1) # "S100a9"        "S100a8"        "Retnlg"        "Il1b"          "Wfdc17"        "Ifitm1"        "Msrb1"         "Csf3r"         "S100a6"        "Ifitm2"  
srat6markers <- FindMarkers(srat_N, ident.1 = 6, min.pct = 0.1) # "Lyz2"          "Ifitm3"        "Apoe"          "Gngt2"         "Cebpb"         "Fcer1g"        "Cst3"          "Cybb"          "Chil3"         "Lgals3"
srat7markers <- FindMarkers(srat_N, ident.1 = 7, min.pct = 0.1) # "Hist1h2ap"     "Hist1h2ae"     "Hist1h1b"      "Mki67"         "Hmgb2"         "Stmn1"         "Pclaf"         "Hist1h1e"      "Top2a"         "Hbb-bt"
srat8markers <- FindMarkers(srat_N, ident.1 = 8, min.pct = 0.1) # "Cst3"           "H2-Ab1"         "Ifi30"          "H2-Eb1"         "Plbd1"          "Tmem176b"       "H2-Aa"          "Ccnd1"          "Tmem176a"
srat9markers <- FindMarkers(srat_N, ident.1 = 9, min.pct = 0.1) # "Ms4a1"         "Plac8"         "Ly6d"          "Cd79a"         "Cd79b"         "Iglc3"         "Iglc2"         "Sp140"         "Mzb1"          "Fcmr"
srat10markers <- FindMarkers(srat_N, ident.1 = 10, min.pct = 0.1) # "Igkv3-4"        "Jchain"         "Ighv1-53"       "Ighm"           "Igkc"           "Ighg2b"         "Igha"           "Iglv2"          "Slpi"
srat11markers <- FindMarkers(srat_N, ident.1 = 11, min.pct = 0.1) # "Vcam1"          "Hmox1"          "Gm26917"        "Cd5l"           "C1qc"           "Apoe"           "Fcna"           "Ctsb"           "Slc40a1"
srat12markers <- FindMarkers(srat_N, ident.1 = 12, min.pct = 0.1) # "Gzma"           "Ncr1"           "Ccl5"           "Klrb1c"         "Nkg7"           "Klrk1"          "AW112010"       "Klra8"          "Klre1"
srat13markers <- FindMarkers(srat_N, ident.1 = 13, min.pct = 0.1) # "Ccl6"          "Ifitm1"        "Prss34"        "Ccl9"          "Cyp11a1"       "Cd63"          "Csf1"          "Hdc"           "Csf2rb"        "Fcer1a" 
...
h5<-LoadH5Seurat("LDSpleenafterQC.h5Seurat") #works, srat_VDJ and h5 look similar (small changes in the order of items though)

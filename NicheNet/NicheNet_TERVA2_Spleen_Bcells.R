...
## sender
sender_celltypes = c("Lef1+ Tcf7+ Cd4+ T cells", "Lef1+ Tcf7+ Cd8a+ Cd8b1+ T cells", "Cd8+ Ccl5+ Nkg7+ Cytotoxic T cells",  "Foxp3+ Regulatory T cells","s100a9+/a8+ Granulocytes")
...
DE_table_all = Idents(Spleen) %>% levels() %>% intersect(sender_celltypes) %>% lapply(get_lfc_celltype, seurat_obj = Spleen, condition_colname = "group_id", condition_oi = condition_oi, condition_reference = condition_reference, expression_pct = 0.10, celltype_col = NULL) %>% reduce(full_join) # use this if cell type labels are the identities of your Seurat object -- if not: indicate the celltype_col properly
...
avg_expression_ligands = AverageExpression(Spleen, features = best_upstream_ligands, assays = "RNA")
...
Lef1Tcf7Cd4Tcells_specific_ligands = sender_ligand_assignment$`Lef1+ Tcf7+ Cd4+ T cells` %>% names() %>% setdiff(general_ligands)
Lef1Tcf7Cd8Tcells_specific_ligands = sender_ligand_assignment$`Lef1+ Tcf7+ Cd8a+ Cd8b1+ T cells` %>% names() %>% setdiff(general_ligands)
Cd8Ccl5Nkg7CytotoxTcells_specific_ligands = sender_ligand_assignment$`Cd8+ Ccl5+ Nkg7+ Cytotoxic T cells` %>% names() %>% setdiff(general_ligands)
Foxp3RegTcells_specific_ligands = sender_ligand_assignment$`Foxp3+ Regulatory T cells` %>% names() %>% setdiff(general_ligands)
Granulocytes_specific_ligands = sender_ligand_assignment$`s100a9+/a8+ Granulocytes` %>% names() %>% setdiff(general_ligands)


ligand_type_indication_df = tibble(
  ligand_type = c(rep("Lef1Tcf7Cd4Tcells_specific_ligands", times = Lef1Tcf7Cd4Tcells_specific_ligands %>% length()),
                  rep("Lef1Tcf7Cd8Tcells_specific_ligands", times = Lef1Tcf7Cd8Tcells_specific_ligands %>% length()),
                  rep("Cd8Ccl5Nkg7CytotoxTcells_specific_ligands", times = Cd8Ccl5Nkg7CytotoxTcells_specific_ligands %>% length()),
                  rep("Foxp3RegTcells_specific_ligands", times = Foxp3RegTcells_specific_ligands %>% length()),
                  rep("Granulocytes_specific_ligands", times = Granulocytes_specific_ligands %>% length()),
                  rep("General", times = general_ligands %>% length())),
  ligand = c(Lef1Tcf7Cd4Tcells_specific_ligands, Lef1Tcf7Cd8Tcells_specific_ligands, Cd8Ccl5Nkg7CytotoxTcells_specific_ligands, Foxp3RegTcells_specific_ligands, Granulocytes_specific_ligands, general_ligands))
...
grid_col_ligand =c("General" = mycolors[[1]],
                   "Lef1Tcf7Cd4Tcells_specific_ligands" = mycolors[[2]],
                   "Lef1Tcf7Cd8Tcells_specific_ligands" = mycolors[[3]],
                   "Cd8Ccl5Nkg7CytotoxTcells_specific_ligands" = mycolors[[4]],
                   "Foxp3RegTcells_specific_ligands" = mycolors[[5]],
                   "Granulocytes_specific_ligands" = mycolors[[6]]
)
...
ligand_order = c(Lef1Tcf7Cd4Tcells_specific_ligands, Lef1Tcf7Cd8Tcells_specific_ligands, Cd8Ccl5Nkg7CytotoxTcells_specific_ligands, Foxp3RegTcells_specific_ligands, Granulocytes_specific_ligands, general_ligands) %>% c(paste(.," ")) %>% intersect(circos_links$ligand)
...
#NOT USEFUL WHEN ONLY THREE INTERACTIONS PASS THE THRESHOLDS 

...
receiver = "Cd4+ Tcells" #Just change this to your cell type of interest
expressed_genes_receiver = get_expressed_genes(receiver, PVAT, pct = 0.10)
background_expressed_genes = expressed_genes_receiver %>% .[. %in% rownames(ligand_target_matrix)]
## sender
sender_celltypes = c("Mgp+ Fibroblasts", "Cd8+ Tcells", "Intermediate Monocytes", "Classical and non-classical Monocytes", "Innate lymphoid cells", "Lgals3+ Macrophages", "Macrophages", "Macrophages activated", "Vascular smooth muscle cells", "Fibroblasts", "Fibroblasts activated", "B cells", "Pi16+ adventitial Fibroblasts", "Granulocytes")
list_expressed_genes_sender = sender_celltypes %>% unique() %>% lapply(get_expressed_genes, PVAT, 0.10) # lapply to get the expressed genes of every sender cell type separately here
expressed_genes_sender = list_expressed_genes_sender %>% unlist() %>% unique()

# Gene set of interest for the receiver population

seurat_obj_receiver= subset(PVAT, idents = receiver)
...
DotPlot(subset(PVAT, idents = sender_celltypes), features = best_upstream_ligands %>% rev(), cols = "RdYlBu") + RotatedAxis()
DotPlot(subset(PVAT, idents = sender_celltypes), features = best_upstream_ligands %>% rev(), cols = "RdYlBu", split.by = "group_id") + RotatedAxis()
...
DE_table_all = Idents(PVAT) %>% levels() %>% intersect(sender_celltypes) %>% lapply(get_lfc_celltype, seurat_obj = PVAT, condition_colname = "group_id", condition_oi = condition_oi, condition_reference = condition_reference, expression_pct = 0.10, celltype_col = NULL) %>% reduce(full_join) # use this if cell type labels are the identities of your Seurat object -- if not: indicate the celltype_col properly
...
avg_expression_ligands = AverageExpression(PVAT, features = best_upstream_ligands, assays = "RNA")
...
MgpFibro_specific_ligands = sender_ligand_assignment$`Mgp+ Fibroblasts` %>% names() %>% setdiff(general_ligands)
...
Lgals3_specific_ligands = sender_ligand_assignment$`Lgals3+ Macrophages` %>% names() %>% setdiff(general_ligands)
...
Fibro_specific_ligands = sender_ligand_assignment$`Fibroblasts` %>% names() %>% setdiff(general_ligands)
Fibroact_specific_ligands = sender_ligand_assignment$`Fibroblasts activated` %>% names() %>% setdiff(general_ligands)
Macroact_specific_ligands = sender_ligand_assignment$`Macrophages activated` %>% names() %>% setdiff(general_ligands)
Pi16Fibro_specific_ligands = sender_ligand_assignment$` Pi16+ adventitial Fibroblasts` %>% names() %>% setdiff(general_ligands)
Granu_specific_ligands = sender_ligand_assignment$`Granulocytes` %>% names() %>% setdiff(general_ligands)


ligand_type_indication_df = tibble(
  ligand_type = c(rep("MgpFibro-specific", times = MgpFibro_specific_ligands %>% length()),
                  rep("Cd8T-specific", times = Cd8T_specific_ligands %>% length()),
                  ...
                  rep("Lgals3-specific", times = Lgals3_specific_ligands %>% length()),
                  ...
                  rep("Fibroact-specific", times = Fibroact_specific_ligands %>% length()),
                  rep("Macroact-specific", times = Macroact_specific_ligands %>% length()),
                  rep("Pi16Fibro-specific", times = Pi16Fibro_specific_ligands %>% length()),
                  rep("Granu-specific", times = Granu_specific_ligands %>% length()),
                  rep("General", times = general_ligands %>% length())),
  ligand = c(MgpFibro_specific_ligands, Cd8T_specific_ligands, IntMono_specific_ligands, Mono_specific_ligands, ILC_specific_ligands, Lgals3_specific_ligands, Macro_specific_ligands, Bcells_specific_ligands, VSMC_specific_ligands, Fibro_specific_ligands, Fibroact_specific_ligands, Macroact_specific_ligands, Pi16Fibro_specific_ligands, Granu_specific_ligands, general_ligands))
...
mycols <- colorBlindness::paletteMartin
displayAllColors(mycols, color = "white")

grid_col_ligand =c("General" = mycols[[1]],
                   "MgpFibro-specific" = mycols[[2]],
                   "Cd8T-specific" = mycols[[3]],
                   "IntMono-specific" = mycols[[4]],
                   "Mono-specific" = mycols[[5]],
                   "ILC-specific" = mycols[[6]],
                   "Lgals3-specific" = mycols[[7]],
                   "Macro-specific" = mycols[[8]],
                   "Bcells-specific" = mycols[[9]],
                   "VSMC-specific" = mycols[[10]],
                   "Fibro-specific" = mycols[[11]],
                   "Fibroact-specific" = mycols[[12]],
                   "Macroact-specific" = mycols[[13]],
                   "MgpFibro-specific" = mycols[[14]],
                   "Granu-specific" = mycols[[15]]
)
grid_col_target =c(
...
ligand_order = c(MgpFibro_specific_ligands, Cd8T_specific_ligands, IntMono_specific_ligands, Mono_specific_ligands, ILC_specific_ligands, Lgals3_specific_ligands, Macro_specific_ligands, Bcells_specific_ligands, VSMC_specific_ligands, Fibro_specific_ligands, Fibroact_specific_ligands, Macroact_specific_ligands, Pi16Fibro_specific_ligands, Granu_specific_ligands, general_ligands) %>% c(paste(.," ")) %>% intersect(circos_links$ligand)
...
gaps = c(rep(width_same_cell_same_ligand_type, times = (circos_links %>% filter(ligand_type == "General") %>% distinct(ligand) %>% nrow() -1)),
         width_ligand_target,
         rep(width_same_cell_same_ligand_type, times = (circos_links %>% filter(ligand_type == "ILC-specific") %>% distinct(ligand) %>% nrow() -1)),
         width_ligand_target
         ) 

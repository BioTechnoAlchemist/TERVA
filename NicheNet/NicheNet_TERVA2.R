...
#TERVA2_harmony <- readRDS("TERVA2_harmony_2.rds")
...
TERVA2_harmony@meta.data$Sample %>% table()
##    LDAO   LDeWAT   LDPVAT LDSpleen     PLAO   PLeWAT   PLPVAT PLSpleen 
##    2687     7991     7343     4231     6691     6412     5384     5258
...
expressed_genes_receiver = get_expressed_genes(receiver, TERVA2_harmony, pct = 0.10)
background_expressed_genes = expressed_genes_receiver %>% .[. %in% rownames(ligand_target_matrix)]
## sender
sender_celltypes = c("Cd4+ Tcells", "Cd8+ Ccl5+ Teffs", "Cd4+ Foxp3+ Tregs", "Natural killer cells", "B cells", "Classical and non-classical Monocytes", "Innate lymphoid cells", "Macrophages", "Macrophages activated", "Fibroblasts", "Vascular smooth muscle cells", "Endothelial cells")
list_expressed_genes_sender = sender_celltypes %>% unique() %>% lapply(get_expressed_genes, TERVA2_harmony, 0.10) # lapply to get the expressed genes of every sender cell type separately here
expressed_genes_sender = list_expressed_genes_sender %>% unlist() %>% unique()

# Gene set of interest for the receiver population

seurat_obj_receiver= subset(TERVA2_harmony, idents = receiver)
...
DotPlot(subset(TERVA2_harmony, idents = sender_celltypes), features = best_upstream_ligands %>% rev(), cols = "RdYlBu") + RotatedAxis()
DotPlot(subset(TERVA2_harmony, idents = sender_celltypes), features = best_upstream_ligands %>% rev(), cols = "RdYlBu", split.by = "group_id") + RotatedAxis()
...
p_ligand_target_network = vis_ligand_target %>% make_heatmap_ggplot("Prioritized ligands","Predicted target genes", color = "dodgerblue",legend_position = "top", x_axis_position = "top",legend_title = "Regulatory potential")  + theme(axis.text.x = element_text(face = "italic", size = 14), axis.text.y = element_text(size = 14), axis.title.x = element_text(size = 14), axis.title.y = element_text(size = 14)) + scale_fill_gradient2(low = "whitesmoke",  high = "dodgerblue", breaks = c(0,0.0045,0.0090))
...
avg_expression_ligands = AverageExpression(TERVA2_harmony, features = best_upstream_ligands, assays = "RNA")
...
ligand_type_indication_df = tibble(
  ligand_type = c(rep("DC2-specific", times = DC2_specific_ligands %>% length()),
                  ...
                  rep("Macro-specific", times = Macroact_specific_ligands %>% length()),
                  rep("General", times = general_ligands %>% length())),
  ligand = c(DC2_specific_ligands, NK_specific_ligands, Cd4T_specific_ligands, Mono_specific_ligands, Macroact_specific_ligands, general_ligands))
...
#
grid_col_ligand =c("General" = "dodgerblue3",
                   "NK-specific" = "royalblue",
                   "DC2-specific" = "darkgreen",
                   "Mono-specific" = "violet",
                   "Macro-specific" = "darkgoldenrod3",
                   "Cd4T-specific" = "pink")
grid_col_target =c(
  "Late_disease-DE" = "tomato")
...
ligand_order = c(Mono_specific_ligands, Macroact_specific_ligands, Cd4T_specific_ligands, NK_specific_ligands,DC2_specific_ligands, general_ligands) %>% c(paste(.," ")) %>% intersect(circos_links$ligand)
...
#  rep(width_same_cell_same_ligand_type, times = (circos_links %>% filter(ligand_type == "DC2-specific") %>% distinct(ligand) %>% nrow() -1)),
#  width_different_cell,
...
#  width_different_cell,
  ...
  width_different_cell,
#  rep(width_same_cell_same_ligand_type, times = (circos_links %>% filter(ligand_type == "Cd4T-specific") %>% distinct(ligand) %>% nrow() -1)),
#  width_different_cell,
  rep(width_same_cell_same_ligand_type, times = (circos_links %>% filter(ligand_type == "General") %>% distinct(ligand) %>% nrow() -1)),
  width_ligand_target
) #If you get a times error here, check with unique(circos_links$ligand_type) which ligand types exist

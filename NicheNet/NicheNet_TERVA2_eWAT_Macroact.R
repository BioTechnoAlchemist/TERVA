...
## sender
sender_celltypes = c("Intermediate Monocytes", "Classical and non-classical Monocytes", "Lgals3+ Macrophages", "Macrophages", "Fibroblasts", "Endothelial cells", "Rgs5+ EC's", "Pi16+ adventitial Fibroblasts", "Vascular smooth muscle cells")
...
ECs_specific_ligands = sender_ligand_assignment$`Endothelial cells` %>% names() %>% setdiff(general_ligands)
...
Rgs5ECs_specific_ligands = sender_ligand_assignment$`Rgs5+ EC's` %>% names() %>% setdiff(general_ligands)
...
ligand_type_indication_df = tibble(
  ligand_type = c(rep("VSMC-specific", times = VSMC_specific_ligands %>% length()),
                  rep("ECs-specific", times = ECs_specific_ligands %>% length()),
                  ...
                  rep("Rgs5ECs-specific", times = Rgs5ECs_specific_ligands %>% length()),
  ...
  ligand = c(VSMC_specific_ligands, ECs_specific_ligands, Fibro_specific_ligands, Lgals3_specific_ligands, Mono_specific_ligands, IntMono_specific_ligands, Rgs5ECs_specific_ligands, Macro_specific_ligands, Pi16Fibro_specific_ligands, general_ligands))
...
grid_col_ligand =c("General" = mycols[[1]],
                   "VSMC-specific" = mycols[[7]],
                   "ECs-specific" = mycols[[3]],
                   "Fibro-specific" = mycols[[4]],
                   "Lgals3-specific" = mycols[[12]],
                   "Mono-specific" = mycols[[6]],
                   "IntMono-specific" = mycols[[2]],
                   "Rgs5ECs-specific" = mycols[[8]],
                   "Macro-specific" = mycols[[9]],
                   "Pi16Fibro_specific_ligands" = mycols[[10]]
)
grid_col_target =c(
  "Late_disease-DE" = mycols[["goldenrod1"]])
...
ligand_order = c(VSMC_specific_ligands, ECs_specific_ligands, Fibro_specific_ligands, Lgals3_specific_ligands, Mono_specific_ligands, IntMono_specific_ligands, Rgs5ECs_specific_ligands, Macro_specific_ligands, Pi16Fibro_specific_ligands, general_ligands) %>% c(paste(.," ")) %>% intersect(circos_links$ligand)
...
gaps = c(
  width_ligand_target,
  rep(width_same_cell_same_ligand_type, times = (circos_links %>% filter(ligand_type == "VSMC-specific") %>% distinct(ligand) %>% nrow() -1)),
  width_different_cell,
  rep(width_same_cell_same_ligand_type, times = (circos_links %>% filter(ligand_type == "Lgals3-specific") %>% distinct(ligand) %>% nrow() -1)),
  width_different_cell,

...
sc <- load10X("/Users/limikk/Desktop/TERVA/TERVA2DATA/PL_AO_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/PL_AO_GEX_Ab_VDJ_noIntr_SC5PPE/count", includeFeatures = "Gene Expression") #This will only include the gene expression data. Include ab data later.
sc
...
sc <- calculateContaminationFraction(sc, list(IG = genes), useToEst = useToEst) #Estimated global contamination fragtion of 5.76%
...
srat_TCR <- import_vdj(input = srat_N, vdj_dir = "~/Desktop/TERVA/TERVA2DATA/PL_AO_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/PL_AO_GEX_Ab_VDJ_noIntr_SC5PPE/vdj_t/", filter_paired = F)
srat_BCR <- import_vdj(input = srat_N, vdj_dir = "~/Desktop/TERVA/TERVA2DATA/PL_AO_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/PL_AO_GEX_Ab_VDJ_noIntr_SC5PPE/vdj_b/", filter_paired = F)

srat_VDJ <- import_vdj(input = srat_N, vdj_dir = "~/Desktop/TERVA/TERVA2DATA/PL_AO_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/PL_AO_GEX_Ab_VDJ_noIntr_SC5PPE/vdj_t/", prefix = "TCR_", filter_paired = F)
srat_VDJ <- import_vdj(input = srat_VDJ, vdj_dir = "~/Desktop/TERVA/TERVA2DATA/PL_AO_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/PL_AO_GEX_Ab_VDJ_noIntr_SC5PPE/vdj_b/", prefix = "BCR_", filter_paired = F)
...
library(rjson)
Tcells <- as.data.frame(fromJSON(file = "~/Desktop/TERVA/TERVA2DATA/PL_AO_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/PL_AO_GEX_Ab_VDJ_noIntr_SC5PPE/vdj_t/cell_barcodes.json"))
names(Tcells)[1] <- "Tcells" 
Bcells <- as.data.frame(fromJSON(file = "~/Desktop/TERVA/TERVA2DATA/PL_AO_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/PL_AO_GEX_Ab_VDJ_noIntr_SC5PPE/vdj_b/cell_barcodes.json"))
...
srat0markers <- FindMarkers(srat_N, ident.1 = 0, min.pct = 0.1) # "Ccl4"          "Cxcl2"         "Ccl3"          "Ccl7"          "Ccl2"          "Cd14"          "Tnf"           "Ccl12"         "Plau"          "Lilrb4a" 
srat1markers <- FindMarkers(srat_N, ident.1 = 1, min.pct = 0.1) # "Cd79a"         "H2-DMb2"       "Fcmr"          "Ly6d"          "Cd79b"         "Ebf1"          "H2-Ob"         "Fcer2a"        "Fcrla"         "Scd1"
srat2markers <- FindMarkers(srat_N, ident.1 = 2, min.pct = 0.1) # "Ms4a4b"        "Cd8b1"         "Cd3d"          "Ly6c2"         "Lef1"          "Cd3e"          "Cd3g"          "Nkg7"          "Lck"           "Lat" 
srat3markers <- FindMarkers(srat_N, ident.1 = 3, min.pct = 0.1) # "Ly6d"          "Cd79a"         "Iglc2"         "Cd79b"         "Fcmr"          "Iglc3"         "Ms4a1"         "Plac8"         "H2-DMb2"       "Mzb1" 
srat4markers <- FindMarkers(srat_N, ident.1 = 4, min.pct = 0.1) # "Cd8b1"         "Ms4a4b"        "Lef1"          "Cd3d"          "Rpl12"         "Rps15a"        "Rps20"         "Gimap6"        "Tmsb10"        "Rps7"          "Rps24"
srat5markers <- FindMarkers(srat_N, ident.1 = 5, min.pct = 0.1) # "Tnfrsf4"       "Ctla4"         "Il2rb"         "Tnfrsf9"       "Ikzf2"         "Hopx"          "Ramp3"         "Tnfrsf18"      "Icos"          "Rora"          "Izumo1r"
srat6markers <- FindMarkers(srat_N, ident.1 = 6, min.pct = 0.1) # "Retnla"         "F13a1"          "Ccl8"           "Selenop"        "Mrc1"           "Folr2"          "Lyve1"          "Ccl6"           "Cd209f"         "C1qa"           "Pltp" 
srat7markers <- FindMarkers(srat_N, ident.1 = 7, min.pct = 0.1) # "Acta2"         "Tagln"         "Myl9"          "Myh11"         "Tpm2"          "Igfbp7"        "Bgn"           "Csrp2"         "Tpm1"          "Cald1"         "Dstn" 
srat8markers <- FindMarkers(srat_N, ident.1 = 8, min.pct = 0.1) # "Fscn1"          "Ccl5"           "Apol7c"         "Ccl22"          "Il12b"          "Epsti1"         "Tbc1d4"         "Serpinb9"       "Serpinb6b"      "Zmynd15"        "Tspan3" 
srat9markers <- FindMarkers(srat_N, ident.1 = 9, min.pct = 0.1) # "Ramp3"          "Cxcr6"          "Lmo4"           "Rora"           "Icos"           "Calca"          "Dgat1"          "Eprs"           "Pxdc1"          "Il7r"           "Il2rb"
srat10markers <- FindMarkers(srat_N, ident.1 = 10, min.pct = 0.1) # "Lyz1"          "Fabp4"         "Lyz2"          "Ctla2a"        "Fabp5"         "Ctsl"          "Mfge8"         "Gpnmb"         "Neat1"         "Gm42418"       "Cfb" 
srat11markers <- FindMarkers(srat_N, ident.1 = 11, min.pct = 0.1) # "Il1b"          "Cd209a"        "Tnip3"         "Ifit1"         "Plbd1"         "Il1r2"         "H2-Eb1"        "Isg15"         "H2-Ab1"        "Anxa2"         "Ifitm2"
srat12markers <- FindMarkers(srat_N, ident.1 = 12, min.pct = 0.1) # "Dmxl1"         "Dennd4a"       "Malat1"        "Malt1"         "Foxp1"         "Satb1"         "Rel"           "Gm26917"       "Bach2"         "Bank1"         "Cerk"
srat13markers <- FindMarkers(srat_N, ident.1 = 13, min.pct = 0.1) # "Cst3"          "Plbd1"         "Ifi205"        "Ppt1"          "Gm2a"          "Naaa"          "Psap"          "Ckb"           "Csf2rb"        "H2-DMb1"       "Il1b"
srat14markers <- FindMarkers(srat_N, ident.1 = 14, min.pct = 0.1) # "Hist1h2ap"     "Pclaf"         "Hmgb2"         "Top2a"         "Mki67"         "H2afx"         "Stmn1"         "Hist1h2ae"     "Tubb5"         "Ighg3"         "Tuba1b" 
srat15markers <- FindMarkers(srat_N, ident.1 = 15, min.pct = 0.1) # "Dcn"           "Gsn"           "Clec3b"        "Serping1"      "Mgp"           "Pi16"          "Serpinf1"      "Igfbp6"        "Cytl1"         "C3"            "Mfap5" 
srat16markers <- FindMarkers(srat_N, ident.1 = 16, min.pct = 0.1) # "S100a9"        "S100a8"        "Acod1"         "Retnlg"        "Il1b"          "Slpi"          "G0s2"          "Hdc"           "Msrb1"         "Clec4e"        "Slc7a11"
srat17markers <- FindMarkers(srat_N, ident.1 = 17, min.pct = 0.1) # "Spp1"          "Gpnmb"         "Cd5l"          "Ctsl"          "Mmp12"         "Ctsd"          "Fabp5"         "Lpl"           "Lgals3"        "Mfge8"         "Apoe"
srat18markers <- FindMarkers(srat_N, ident.1 = 18, min.pct = 0.1) # "Igkv1-110"     "Jchain"        "Igha"          "Igkv10-96"     "Iglv1"         "Igkc"          "Igkv3-2"       "Iglv3"         "Ighg2b"        "Slpi"          "Ighm"
srat19markers <- FindMarkers(srat_N, ident.1 = 19, min.pct = 0.1) # "Siglech"       "Bst2"          "Cox6a2"        "Ly6c2"         "Cd7"           "Klk1"          "Ccr9"          "Irf8"          "Ly6d"          "Tyrobp"        "Lrp8"
srat20markers <- FindMarkers(srat_N, ident.1 = 20, min.pct = 0.1) # "Mki67"          "Cxcl10"         "Ube2c"          "Stmn1"          "Top2a"          "Ccl2"           "Tuba1b"         "Ccl4"           "Birc5"          "Cxcl1"          "Tnf"
...
h5<-LoadH5Seurat("PLAOafterQC.h5Seurat") #works, srat_VDJ and h5 look similar (small changes in the order of items though)

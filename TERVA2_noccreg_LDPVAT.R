...
sc <- load10X("/Users/limikk/Desktop/TERVA/TERVA2DATA/LD_PVAT_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/LD_PVAT_GEX_Ab_VDJ_noIntr_SC5PPE/count", includeFeatures = "Gene Expression") #This will only include the gene expression data. Include ab data later.
sc

#Channel with 32285 genes and 8102 cells
...
#1477 genes passed tf-idf cut-off and 612 soup quantile filter.  Taking the top 100.
#Using 418 independent estimates of rho.
...
sc <- calculateContaminationFraction(sc, list(IG = genes), useToEst = useToEst) #Estimated global contamination fragtion of 6.07%
...
#### ADD antibody data ####
tenx <- Read10X("/Users/limikk/Desktop/TERVA/TERVA2DATA/LD_PVAT_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/LD_PVAT_GEX_Ab_VDJ_noIntr_SC5PPE/count/filtered_feature_bc_matrix/")
...
plot1 <- DimPlot(srat, group.by="DF.classifications_0.25_0.17_349", reduction="umap", pt.size=1, order=c("Coll.Duct.TC","Doublet"), cols=c("#66C2A5","#FFD92F","#8DA0CB"))
plot2 <- DimPlot(srat, reduction = "umap", pt.size=1, group.by = "RNA_snn_res.0.6")

plot1 + plot2 


srat_DF <- subset(srat, subset = DF.classifications_0.25_0.17_349 == "Singlet") #Creates a new Seurat Object from the singlets.

plot3 <- DimPlot(srat_DF, group.by="DF.classifications_0.25_0.17_349", reduction="umap", pt.size=0.5, order=c("Coll.Duct.TC","Doublet"), cols=c("#66C2A5","#FFD92F","#8DA0CB"))
...
srat_TCR <- import_vdj(input = srat_N, vdj_dir = "~/Desktop/TERVA/TERVA2DATA/LD_PVAT_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/LD_PVAT_GEX_Ab_VDJ_noIntr_SC5PPE/vdj_t/", filter_paired = F)
srat_BCR <- import_vdj(input = srat_N, vdj_dir = "~/Desktop/TERVA/TERVA2DATA/LD_PVAT_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/LD_PVAT_GEX_Ab_VDJ_noIntr_SC5PPE/vdj_b/", filter_paired = F)

srat_VDJ <- import_vdj(input = srat_N, vdj_dir = "~/Desktop/TERVA/TERVA2DATA/LD_PVAT_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/LD_PVAT_GEX_Ab_VDJ_noIntr_SC5PPE/vdj_t/", prefix = "TCR_", filter_paired = F)
srat_VDJ <- import_vdj(input = srat_VDJ, vdj_dir = "~/Desktop/TERVA/TERVA2DATA/LD_PVAT_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/LD_PVAT_GEX_Ab_VDJ_noIntr_SC5PPE/vdj_b/", prefix = "BCR_", filter_paired = F)
...
library(rjson)
Tcells <- as.data.frame(fromJSON(file = "~/Desktop/TERVA/TERVA2DATA/LD_PVAT_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/LD_PVAT_GEX_Ab_VDJ_noIntr_SC5PPE/vdj_t/cell_barcodes.json"))
names(Tcells)[1] <- "Tcells" 
Bcells <- as.data.frame(fromJSON(file = "~/Desktop/TERVA/TERVA2DATA/LD_PVAT_GEX_Ab_VDJ_noIntr_SC5PPE/outs/per_sample_outs/LD_PVAT_GEX_Ab_VDJ_noIntr_SC5PPE/vdj_b/cell_barcodes.json"))
...
srat0markers <- FindMarkers(srat_N, ident.1 = 0, min.pct = 0.1) # Mfap4, Cilp, Sfrp2, Fbln1, Sfrp4,C7, Gas1, Cygb, Lum, Bgn, Dcn
srat1markers <- FindMarkers(srat_N, ident.1 = 1, min.pct = 0.1) # Ccl8, C1qa, C1qc, C1qd, Retnla, Pf4, Mrc1, F13a1, Apoe
srat2markers <- FindMarkers(srat_N, ident.1 = 2, min.pct = 0.1) # Apod, Cxcl14, Smoc2, Gdf10, F3, Fbln1, C7, Cxcl12, Cygb, Gpx3
srat3markers <- FindMarkers(srat_N, ident.1 = 3, min.pct = 0.1) # Ccl5, Ms4a4b, Cd3d, Cd3e, Cd8b1, Cd3g
srat4markers <- FindMarkers(srat_N, ident.1 = 4, min.pct = 0.1) # Cd79a, Lyd6, Cd79b, Iglc2, Fcmr
srat5markers <- FindMarkers(srat_N, ident.1 = 5, min.pct = 0.1) #
srat6markers <- FindMarkers(srat_N, ident.1 = 6, min.pct = 0.1) 
srat7markers <- FindMarkers(srat_N, ident.1 = 7, min.pct = 0.1) # Fabp4, Gpihbp1, Cd36, Egfl7, Pecam1, Ptprb
...
h5<-LoadH5Seurat("LDPVATafterQC.h5Seurat") #works, srat_VDJ and h5 look similar (small changes in the order of items though)

...
data_up <- read.csv("upregulated results/eWAT_Fibro_Mgp_gProfiler_mmusculus_26-10-2023_13-57-11__intersections.csv", sep = ";") #This originates from a web interface run of g:profiler at https://biit.cs.ut.ee/gprofiler/gost.
data_up <- data_up %>%
...
data_down <- read.csv("downregulated results/eWAT_MgpFibro_gProfiler_mmusculus_30-04-2024_13-45-28__intersections.csv", sep = ",")
data_down <- data_down %>%
...
gp_mod_enrich_up <- enrichDF2enrichResult(enrichDF = gp_mod_up, keyColname = "term_id", geneColname = "gene_name", 
                                          pvalueColname = "adjusted_p_value", descriptionColname = "term_name_mod", pvalueCutoff = 0.05)
...
up2 <- cnetplot(gp_mod_enrich_up, showCategory = 5, colorEdge = TRUE, color.params = list(edge = "#005C5C")) + 
